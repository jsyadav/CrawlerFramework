...
log = logging.getLogger('PpcgeeksForumsConnector')
class PPCGeeksForumsConnector(BaseConnector):
    '''Forum for BB Geeks
    '''
    @logit(log , 'fetch')
    def fetch(self):
        """Fetch of http://forums.ppcgeeks.com/
        """
        try:
    ...
    def __addThreadAndPosts(self):
        ...
        try:
            ...
            while self.__iteratePosts():
                try:
                    self.currenturi = self.soup.find('a', rel='prev')['href']
                    self.__setSoupForCurrentUri()
                except:
    ...
    def __getThreads(self):
            ...
            for thread in threads:
                ...
                try:
                    self.__links_to_process.append(thread.find('a', id=re.compile('thread_title_\d+'))['href'])
                except:
    ...
    def __getData(self, post, is_question):
        ...
        try:
            date_str = re.sub('\s+', ' ', stripHtml(post.find('td', 'thead').findAll('div', 'normal')[-1].renderContents()))
