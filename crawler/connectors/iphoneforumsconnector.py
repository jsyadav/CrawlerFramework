...
log = logging.getLogger('IphoneForumsConnector')
class IphoneForumsConnector(BaseConnector):
    @logit(log , 'fetch')
    def fetch(self):
        '''This is a fetch method which  fetches the data 
        sample url:http://www.iphoneforums.net/forum/iphone-4-apps-20/
        '''
        try:
            self.__setSoupForCurrentUri()
            posts = self.soup.find('div', id='posts')
            threads = self.soup.find('table',id='threadslist')
            if posts:
                return self.__addThreadAndPosts()
            elif threads:
                return self.__createTasksForThreads()
            else:
                log.info('not correct url')
    ...
    def __processForumUrl(self):
        ...
        and add the tasks
        """
        threads = self.soup.find('table',id='threadslist').findAll('tr')
        for thread in threads[1:]:
            try:
                ...
                if  self.__total_threads_count > self.__max_threads_count:
                    ...
                    return False  
                try:
                    date_tag = stripHtml(thread.find('td', id = re.compile('td_title_\d+')).\
                                findNext('td').renderContents()).splitlines()[0].strip()
                ...
                try:
                    s = thread.find('a', id = re.compile('thread_title_\d+'))['href']                                      
    ...
    def __setHierarchy(self):
        ...
        page = {}
        try: 
            hierarchies = stripHtml(self.soup.find('div','page').find('div','smallfont').\
                            renderContents())
            self.hierarchy = [x.strip() for x in hierarchies.split('>')]
            page['data'] = page['title'] = self.hierarchy[-1].strip().splitlines()[-1]
        ...
        if checkSessionInfo(self.genre, self.session_info_out, self.task.instance_data['uri'],\
            ...
            return False
        try:
            date_str = self.soup.find('a',attrs = {'name':re.compile('post\d+')}).\
                        next.next.strip()
    ...
    def __goToLastPage(self):
        ...
        try:
            ...
            if last_page_tag:
                ...
                last_page_tag = pagination_tag.find('a',title = re.compile('Next Page - '))
                if last_page_tag:
                    uri = last_page_tag['href']
            if not uri:
    ...
    def __getData(self, post, is_question):
        ...
        try:
            page['title'] = self.hierarchy[-1].strip().splitlines()[-1]
            if not is_question:
        ...
        try:
            ...
            previous_message_tag = data_tag.find('td','quote')
            if previous_message_tag:
                previous_message_tag.extract()
        ...
        try:              
            author_info = [stripHtml(each.renderContents())for each in post.\
                        find('div', id = re.compile('postmenu_\d+')).findParent('td').\
                        findAll('div','smallfont')][-1].splitlines()
