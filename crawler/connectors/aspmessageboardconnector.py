...
log = logging.getLogger('AspMessageConnector')
class AspMessageBoardConnector(BaseConnector):
    '''
    This will fetch the info for aspmessageboard.com forums
    Sample uris is
    http://www.aspmessageboard.com/forumdisplay.php?f=17
    ...
    def fetch(self):
        ...
        try:
            ...
            log.info(self.parent_uri)
            if self.currenturi.startswith('http://www.aspmessageboard.com/showthread.'):
                ...
                while True:
                    self.__addPosts()
                    try:
                        self.currenturi = 'http://www.aspmessageboard.com/' + self.soup.find('a',text='&gt;').parent['href']
                    except:
                    ...
                    if not self.__setSoup():
                        ...
                        break
                return True
            elif self.currenturi.startswith('http://www.aspmessageboard.com/forumdisplay'):
                self.total_posts_count = 0
                self.last_timestamp = datetime( 1980,1,1 )
                self.max_posts_count = int(tg.config.get(path='Connector',key='aspmessageboard_numresults'))
    ...
    def __getThreads( self ):
        ...
        for thread in threads:
            ...
            try:
                ...
                temp_task.instance_data[ 'uri' ] = normalize( 'http://www.aspmessageboard.com/' + thread.find('a',id=re.compile('^thread_title_.*$'))['href'] )
