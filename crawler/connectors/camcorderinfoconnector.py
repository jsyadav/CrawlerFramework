...
log = logging.getLogger('CamcorderinfoConnector')
class CamcorderinfoConnector(BaseConnector):
    ...
    def fetch(self):
        try:
            ...
            post_count = 0
            self.POSTS_ITERATIONS = tg.config.get(path='Connector',key='camcorderinfo_numposts') 
            ...
            if last_page:
                log.info(self.log_msg('last_page found : %s'%'http://www.camcorderinfo.com/bbs/'+last_page['href']))
                self.currenturi ='http://www.camcorderinfo.com/bbs/' +  last_page['href']
                res=self._getHTML()
                self.rawpage=res['result']
                self._setCurrentPage()
            elif navbar:
                 links =  navbar.parent.parent.findAll('td',{'class':re.compile('^alt[0-9]+')},text=re.compile('[0-9]+'))
                 self.currenturi = 'http://www.camcorderinfo.com/bbs/'+links[-1].parent['href']
            ...
            while True:
                ...
                posts = self.soup.findAll('table',{'id':re.compile('post[0-9]+'),'class':'tborder'})
                ...
                for index,post in enumerate(posts):
                    ...
                    try:
                        page = {}
                        try:
                            data = stripHtml(post.find('td',{'class':'alt1'}).find(lambda tag : tag.name=='div' and not \
                                                                                       tag.attrs).renderContents())
...
#                            page['data'] =  stripHtml(re.sub('Quote:','',data).strip())
                        ...
                        try:
                            page['et_author_profile']='http://www.camcorderinfo.com/bbs/'+author_info.find\
                        ...
                        try:
                            author_joined_date = post.find('div',text=re.compile('Join Date:')).replace('Join Date:','').strip()
                            author_joined_date = datetime.strptime(author_joined_date,'%b %Y')
                        ...
                        try:
                            author_posts = re.sub('Posts:|,','',post.find('div',text=re.compile('Posts:'))).strip()
                            page['ei_author_posts_count'] = int(author_posts)
                        except:
                            log.info(self.log_msg('could not parse author posts count'))
                        try:
                            author_location = post.find('div',text=re.compile('Location:')).replace('Location:','').strip()
                            page['et_author_location'] = author_location
                        except:
                            log.info(self.log_msg('could not parse author location'))
                        try:
                            post_permalink = 'http://www.camcorderinfo.com/bbs/' + post.find('a',{'target':'new'})['href']
                ...
                if prev_page:
                    ...
                    self.currenturi = 'http://www.camcorderinfo.com/bbs/'+prev_page['href']
                    res=self._getHTML()
                    self.rawpage=res['result']
                    self._setCurrentPage()
                else:
