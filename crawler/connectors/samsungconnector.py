...
log = logging. getLogger("SamsungConnector")

class SamsungConnector(BaseConnector):    
    """Connector for samsung.com 
    """
    @logit(log,'fetch')
    def fetch(self):
        """
        It fetches the data from the url mentioned
        sample url :http://www.samsung.com/us/video/tvs/UN55D6900WFXZA-reviews
        """
        self.__genre = "Review"
    ...
    def __addReviews(self):
        ...
        for review in reviews:
            ...
            rating = {'ef_rating_overall':'BVRRRatingOverall_Review_Display',\
                        ...
                        'ef_rating_performance':'BVRRRatingPerformance_Review_Display',\
                        'ef_rating_design':'BVRRRatingDesign_Review_Display',\
            ...
            try:
                recommended_tag = review.find('span','BVDI_FVVote BVDI_FVPositive BVDI_FVLevel2')
                if recommended_tag:
                    page['ei_data_helpful_count'] = int(stripHtml(recommended_tag.find('span','BVDINumber').renderContents()))
            except:
