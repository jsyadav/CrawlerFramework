...
log = logging.getLogger('DvinfoConnector')
class DvinfoConnector(BaseConnector):
    ...
    def fetch(self):
        try:
            ...
            post_count = 0
            self.POSTS_ITERATIONS = tg.config.get(path='Connector',key='dvinfo_numposts') 
...
#                            page['data'] =  stripHtml(re.sub('Quote:','',data).strip())
                        ...
                        try:
                            post_permalink = post.find('a',{'id':re.compile('postcount')})['href']
                            post_id = re.search(r'.*?([0-9]+)-post[0-9]+.html',post_permalink).group(1)
                        except:
                            ...
                            if result['updated']:
                                ...
                                try:
                                    ...
                                    if date.split(',')[0].strip() == 'Yesterday':
                                        ...
                                        posted_date = datetime.strptime(re.sub('([a-z]+?),',',',date) ,'%B %d, %Y, %I:%M %p')
            ...
            try:
                ...
                if not checkSessionInfo(self.genre, self.session_info_out, 
                    ...
                    if result['updated']:
                        ...
                        page['uri_domain'] = urlparse.urlparse(page['uri']).hostname
                        page['priority']=self.task.priority
                        page['level']=self.task.level
                        page['pickup_date'] = datetime.strftime(datetime.utcnow(),"%Y-%m-%dT%H:%M:%SZ")
                        page['posted_date'] = datetime.strftime(datetime.utcnow(),"%Y-%m-%dT%H:%M:%SZ")
