...
log = logging.getLogger('GradSchoolForumConnector')
class GradSchoolForumConnector(BaseConnector):
    '''
    This will fetch the info for smallbizserver.net forums
    Sample uris is
    http://www.gradschoolforum.com/f12-application-questions-and-admissions-advice.html
    '''
    @logit(log , 'fetch')
    def fetch(self):
        """
        Fetch of http://gradschoolforum.com
        """
        self.genre="Review"
        try:
            
            self.parent_uri = self.currenturi
            if self.currenturi.startswith('http://www.gradschoolforum.com/t') \
                    or self.currenturi.startswith('http://www.gradschoolforum.com/showthread.'):\
                ...
                while True:
                    self.__addPosts()
                    try:
                        self.currenturi = 'http://www.gradschoolforum.com/' + self.soup.find('a',text='&gt;').parent['href']
                    except:
                    ...
                    if not self.__setSoup():
                        ...
                        break
                return True
            elif self.currenturi.startswith('http://www.gradschoolforum.com/f') or 'searchid=' in self.currenturi:
                self.total_posts_count = 0
                self.last_timestamp = datetime( 1980,1,1 )
                self.max_posts_count = int(tg.config.get(path='Connector',key='gradschoolforum_numresults'))
    ...
    def __getThreads( self ):
        ...
        for thread in threads:
            ...
            try:
                ...
                temp_task.instance_data[ 'uri' ] =  re.sub('\?s=.*?&','?',normalize( 'http://www.gradschoolforum.com/' + thread.find('a',id=re.compile('^thread_title_.*$'))['href'] ))
