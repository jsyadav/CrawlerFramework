...
log = logging.getLogger('CustomerServiceScoreBoardConnector')
class CustomerServiceScoreBoardConnector(BaseConnector):
    '''
    This will fetch the info for 
    Sample uris is
    http://www.customerservicescoreboard.com/Bank+of+America
    '''
    @logit(log , 'fetch')
    def fetch(self):
    ...
    def __iteratePosts(self):
        ...
        """
        try:
            comment_rows = self.soup.findAll('div','tab_content')
            for comment_row in comment_rows:
                posts = comment_row.findAll('li')[1:]
                log.info(len(posts))
                if not posts:
    ...
    def __getData(self, post):
        ...
        try:
            ...
            return
        try:
            date_str = stripHtml(info_tag.find('span').next.next.__str__()).strip()
            page['posted_date']= datetime.strptime(date_str,"%m/%d/%y %I:%M%p").strftime("%Y-%m-%dT%H:%M:%SZ")
        except:
            log.exception(self.log_msg('posted_date not be found in %s'% self.currenturi))
            page['posted_date'] = datetime.strftime(datetime.utcnow(),"%Y-%m-%dT%H:%M:%SZ")
        
        try:
            page['et_author_name'] = stripHtml(info_tag.find('span').renderContents())
        except:
