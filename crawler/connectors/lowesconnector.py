...
log = logging.getLogger('LowesConnector')
class LowesConnector(BaseConnector):
    ...
    def fetch(self):
        ...
        try:
            ...
            self.currenturi = self.soup.find('iframe', width = '100%')['src'].split('?')[0]
            self.__setSoupForCurrentUri()
...
##                    break
    ...
    def __getData(self, post):
        ...
        try:
            page['et_data_recommended'] = stripHtml(post.find('div','BVRRWouldRecommend').\
                                                renderContents())
        except:
        ...
        try:
            page['et_author_level'] = stripHtml(post.find('span','BVRRValue BVRRContextDataValue BVRRContextDataValueExpertise').\
                                            renderContents())                            
        except:
            log.exception(self.log_msg('author location not found'))     
        try:
            page['ef_rating_overall'] =  float(post.find('img',' rating')['title'].split('/')[0])
        except:
            log.exception(self.log_msg('rating overall not found'))    
        try:
            page['et_data_pros'] = stripHtml(post.find('span','BVRRValue BVRRReviewProTags').\
                            renderContents())
        except:
            log.exception(self.log_msg('data pros not found %s'%self.currenturi))
        try:
            page['et_data_cons'] = stripHtml(post.find('span','BVRRValue BVRRReviewConTags').\
                            renderContents())
        except:
            log.exception(self.log_msg('data pros not found %s'%self.currenturi))  
        try:
            prod_list = prod_list =[each.findParent('div')for each in post.findAll('div','BVRRLabel BVRRRatingNormalLabel')]
            for each in prod_list:
                entity =stripHtml( each.find('div','BVRRLabel BVRRRatingNormalLabel').renderContents())
                page['ef_rating_'+ entity.lower().replace(' ','_').replace(':','')] = \
                                  float(each.find('img')['title'].split('/')[0])
        except:
