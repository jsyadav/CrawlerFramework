...
log = logging.getLogger('MacysConnector')
class MacysConnector(BaseConnector):
    
    @logit(log , 'fetch')
    def fetch(self):
        '''This is a fetch method which  fetches the data 
        sample url: http://reviews.macys.com/7129/202250/lauren-by-ralph-lauren-overcoat-columbia-reviews/reviews.htm
        '''
        try:
            self.genre = "Review"
