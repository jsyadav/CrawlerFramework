...
log = logging.getLogger('ReSellerRatingsConnector')
class ReSellerRatingsConnector(BaseConnector):
    
    @logit(log , 'fetch')
    def fetch(self):
        '''This is a fetch method which  fetches the data 
        sample url: http://www.resellerratings.com/store/Dick_s_Sporting_Goods
        '''
        try:
            self.genre = "Review"
            self.baseuri = 'http://www.resellerratings.com'
            ...
            while self.__iteratePosts():
                self.currenturi = self.baseuri + main_page_soup.find('a',rel ='next')['href']
                self.__setSoupForCurrentUri()
                main_page_soup = copy.copy(self.soup)
##                count+=1
