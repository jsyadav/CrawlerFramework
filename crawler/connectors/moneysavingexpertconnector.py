...
log = logging.getLogger('MoneySavingExpertConnector')
class MoneySavingExpertConnector(BaseConnector):
    @logit(log , 'fetch')
    def fetch(self):
        ...
        try:
            ...
            if 'forumdisplay.php' in self.currenturi or 'search.php' in self.currenturi:
    ...
    def __addThreadAndPosts(self): 
        ...
        try:
            self.genre = "Review"
            self.__hierarchy = []
            self.__baseuri = 'http://forums.moneysavingexpert.com/'
            ...
            while self.__iteratePosts():
                try:
                    next_page_uri = main_page_soup.find('a', text='&lt;',rel='prev').parent['href']
        ...
        try:
            ...
            while self.__processForumUrl():
                try:
                    ...
                    if not 'search.php' in self.task.instance_data['uri']:
                        self.currenturi = self.__baseuri + 'forumdisplay.php?'+ urlencode(data_dict)                    
                    else:
                        self.currenturi =  self.__baseuri + 'search.php?' + urlencode(data_dict)                 
                    self.__setSoupForCurrentUri()
                except:
        ...
        for thread in threads:
            try:
                ...
                try:    
                    ...
                    log.info(self.log_msg('last post author not found in the url\
                                                    %s'%self.currenturi))                    
                try:    
    ...
    def __iteratePosts(self):   
        ...
        try:
            #posts = self.soup.findAll('table', id=re.compile('post\d+'))
    ...
    def __setParentPage(self):
        ...
        page = {}
        try: 
            hierarchies = stripHtml(self.soup.find('div',id ='msebreadcrumb').renderContents())
            hierarchy = [x.strip() for x in hierarchies.split('>')]
            self.hierarchy = [x.strip() for x in hierarchies.split('>')]
            page['data'] = page['title'] = hierarchy[-1].split('\r')[-2]
...
##            thread_stat = data_tag.find('div',id ='MSEThreadStats')
##            if thread_stat :
##                thread_stat.parent.findParent('div').extract()
                    
            #[x.extract() for x in data_tag.findNext('div')]
        ...
        try:
            ...
            if date_str.startswith('Today'):
                ...
                date_str= date_str.replace('Yesterday', datetime.strftime((datetime.utcnow()-timedelta(days=1)),'%m-%d-%Y'))            
        ...
        try:              
            author_info = [each.strip() for each in stripHtml(post.find('br').
                           findNext('div').renderContents()).splitlines()
                                                 if each.strip()] 
            #log.info(self.log_msg('author_info not found'))      
            for each in author_info:
                try:
                    if each.startswith('Join Date:'):
                        join_date = '01 ' + each.replace('Join Date:','').strip()
                        
                        page['edate_author_joined_date'] =datetime.strftime\
                        (datetime.strptime(join_date,'%d %b %Y'),"%Y-%m-%dT%H:%M:%SZ")
                        log.info(page['edate_author_joined_date'])  # Convert Date str to Date Time Format
                    if each.startswith('Location:'):
                        page['et_author_location'] = each.replace('Location:',\
                                                        '').strip() 
                    if each.startswith('Age:'):
                        page['ei_author_age'] = int(each.replace('Age:',\
                                                        '').strip()) 
                    if each.startswith('Posts:'):
                        page['ei_author_posts_count'] = int(each.replace\
                                        ('Posts:','').replace(',','').strip()) 
                    if each.startswith('Salute:'):
                        page['ei_author_salute_count'] = int(each.replace\
                                                    ('Salute:','').replace(',','').strip())
                    if each.startswith('Location:'):
                        page['et_author_location'] = each.replace('Location:',\
                                                        '').strip()                               
                except:
