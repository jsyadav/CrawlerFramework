...
log = logging.getLogger('BroncosFreaksConnector')
class BroncosFreaksConnector(BaseConnector):
    @logit(log , 'fetch')
    def fetch(self):
        try:
        ...
        try:
            ...
            while True:
                ...
                if not self.__iteratePosts():
                    break
                try:
                    self.currenturi = main_page_soup.find('a',text='&lt').parent['href']
                    ...
                    if date_tag.startswith('Today'):
                        ...
                        date_tag = date_tag.replace('Yesterday', datetime.\
                                    strftime((datetime.utcnow()-timedelta\
                                         (days=1)),'%m-%d-%Y'))                                   
                    thread_time = datetime.strptime(date_tag,'%m-%d-%Y  %I:%M %p')
                except:
    ...
    def __getData(self, post, is_question):
        ...
        if not page['title'] and not page['data']:
            ...
            return False                    
        try:
            date_str = stripHtml(post.find('td').renderContents())
            if date_str.startswith('Today'):
                date_str = date_str.replace('Today', datetime.strftime\
                (datetime.utcnow(),'%m-%d-%Y'))            
            elif date_str.startswith('Yesterday'):
            ...
            for each in author_info:
                try:
                    if each.startswith('Join Date:'):
                        ...
                        (datetime.strptime(join_date,'%d %b %Y'),"%Y-%m-%dT%H:%M:%SZ")  # Convert Date str to Date Time Format
