...
log = logging. getLogger("SonyConnector")
...
class SonyConnector(BaseConnector):    
    ...
    def fetch(self):
        ...
        sample url :http://store.sony.com/webapp/wcs/stores/servlet/ProductDisplay?catalogId=10551&storeId=10151&langId=-1&productId=8198552921666312874#reviews
        """
        self.__genre = "Review"
        ...
        try:
            ...
            product_id = self.currenturi.split('productId=')[-1].split('#')[0]
            if product_id:
                self.currenturi = 'http://reviews.store.sony.com/5050hosted-en_us/'+product_id+'/reviews.djs?format=embeddedhtml'
        ...
        for review in reviews:
            ...
            try:
                ...
                log.exception(self.log_msg("Author Profile cannot be fetched for url\
                                                                %s"%page['uri']))
            try:
                helpful_tag = review.find('span','BVDI_FVPositive BVDI_FVLevel2')
                if helpful_tag:
                    page['ei_data_helpful_count'] = int(stripHtml(helpful_tag.\
                        find('span','BVDIValue BVDINumber').renderContents()))
            except:
                log.exception(self.log_msg("data helful count not fetched for the uri %s"%page['uri']))                                                        
        
            pros_cons = {'et_data_cons':'BVRRValue BVRRReviewCons', 'et_data_pros':'BVRRValue BVRRReviewPros'}
