...
log = logging.getLogger('LegalBeaglesConnector')
class LegalBeaglesConnector(BaseConnector):
    @logit(log , 'fetch')
    def fetch(self):
    ...
    def __addThreadAndPosts(self): 
        ...
        try:
            self.genre = "Review"
            self.__hierarchy = []
            self.__baseuri = 'http://www.legalbeagles.info/forums/'
            ...
            try:
                ...
                try:
                    ...
                    if date_tag.startswith('Today'):
                        ...
                        thread_time = datetime.strptime(re.sub("(\d+)(st|nd|rd|th)",r"\1",date_tag).strip(),"%d %B %Y  %H:%M:%p") 
                except:
        ...
        try:
            ...
            if date_str.startswith('Today'):
                ...
                page['posted_date'] = datetime.strftime(datetime.strptime(re.sub("(\d+)(st|nd|rd|th)",r"\1",date_str).\
                                        strip(),"%d %B %Y, %H:%M:%p"),"%Y-%m-%dT%H:%M:%SZ") 
        except:
                ...
                try:
                    ...
                    if each.startswith('Posts:'):
                        page['ei_author_posts_count'] = int(each.replace\
                                        ('Posts:','').replace(',','').strip()) 
                    if each.startswith('Blog Entries:'):
                        page['et_author_blog_entries'] = each.replace\
                                                    ('Blog Entries:','').replace(',','').strip()
                    if each.startswith('Thanks:'):
                        page['et_author_thanks'] = each.replace\
                                                    ('Thanks:','').replace(',','').strip()     
                    if each.startswith('Thanked'):
                        page['et_author_thanked'] = each.split('Time')[0].replace('Thanked','').strip()                                                                
