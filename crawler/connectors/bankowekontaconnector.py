...
log = logging.getLogger('BankOweKontaConnector')

class BankOweKontaConnector( BaseConnector ):
    '''
    A Connector to find the data in
    test with the sample uris
    http://bankowe-konta.info/citibank.php
    '''
    @logit(log,"fetch")
    def fetch(self):
        """
        ...
        self.genre = 'Review'
        try:
            #self.currenturi = 'http://bankowe-konta.info/citibank.php'
            self.base_url = 'http://bankowe-konta.info'
...
##            log.info(self.log_msg('User Raing not found cannot be found'))
    ...
    def __addReviews(self):
        ...
        """
        try:
            reviews = self.soup.findAll('div','kom3')
        except:
        ...
        for review in reviews:
            ...
            try:
                page['posted_date'] = datetime.strftime(datetime.utcnow(),"%Y-%m-%dT%H:%M:%SZ")
                kom1_tag = review.find('div','kom1')
                author_tag = kom1_tag.find('b').extract()
                page['et_author_name'] = stripHtml(author_tag.renderContents())
                date_str = stripHtml(kom1_tag.renderContents())
                page['posted_date'] = datetime.strftime( datetime.strptime(date_str,'- %m/%d/%Y - %H:%M'),"%Y-%m-%dT%H:%M:%SZ")
            except:
                log.exception(self.log_msg('author name or posted_date not found'))
            try:
                data_tag = review.find('div','kom2')
                title_tag = data_tag.find('b').extract()
