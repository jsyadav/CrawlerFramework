...
log = logging.getLogger('AvforumsConnector')
class AvforumsConnector(BaseConnector):
    ...
    def fetch(self):
        try:
            ...
            post_count = 0
            self.POSTS_ITERATIONS = tg.config.get(path='Connector',key='avforums_numposts') 
            ...
            navbar = self.soup.find('td',{'class':'vbmenu_control'},text=re.compile('Page [0-9]+'))
            if last_page:
                log.info(self.log_msg('last_page found : %s'%last_page['href']))
                self.currenturi = last_page['href']
                res=self._getHTML()
                self.rawpage=res['result']
                self._setCurrentPage()
            elif navbar:
                 links =  navbar.parent.parent.findAll('td',{'class':re.compile('^alt[0-9]+')},text=re.compile('[0-9]+'))
                 self.currenturi = links[-1].parent['href']
                 log.info(self.log_msg('last_page as last link in navbar: %s'%self.currenturi))
                 res=self._getHTML()
                 self.rawpage=res['result']
                 self._setCurrentPage()
            else:
                log.info(self.log_msg('only one page present for this thread'))
            while True:
...
#                            page['data'] =  stripHtml(re.sub('Quote:','',data).strip())
                        ...
                        try:
                            page['et_author_profile']=author_info.find('a',{'class':'bigusername'})['href']
                        except:
                        ...
                        try:
                            author_feedback = stripHtml(author_metadata.find('div',text=re.compile('iTrader:')).parent.renderContents())
                            author_feedback =  re.findall(re.compile(r'iTrader:.*?([0-9]+).*',re.DOTALL),author_feedback)[0]
                            page['ei_author_feedback_positive'] = int(author_feedback)
                        except:
                            log.info(self.log_msg('could not parse author positive feedback'))
                        try:
                            appreciation = post.find('div',{'class':'postbit'}).renderContents()
                            appreciation = re.search('Thanks: Gave ([0-9]+), Got ([0-9]+)$',appreciation).groups()
                            page['ei_author_thanks_gave'] = int(appreciation[0])
                            page['ei_author_thanks_got'] = int(appreciation[1])
                        except:
                            log.info(self.log_msg('could not parse author thanks field'))
                        try:
                            post_permalink = post.find('a',text='permalink').parent['href']
                        except:
                        ...
                        try:
                            ...
                            continue
                        log.info(self.log_msg(checkSessionInfo(self.genre, self.session_info_out,
                        ...
                        if not checkSessionInfo(self.genre, self.session_info_out,
                            ...
                            result=updateSessionInfo(self.genre, self.session_info_out,post_permalink, post_hash, 
                                                     'Thread', self.task.instance_data.get('update'), 
                                                     parent_list=parent_list)
                            if result['updated']:
                                page['parent_path']=copy.copy(parent_list)
                                parent_list.append(post_permalink)
                                ...
                                page['uri_domain'] = urlparse.urlparse(post_permalink).hostname
                                ...
                                try:
                                    ...
                                    if date.split(',')[0].strip() == 'Yesterday':
                                        ...
                                        posted_date = datetime.strptime(re.sub('([a-z]+?),',',',date) ,'%d-%m-%Y, %I:%M %p')
                ...
                if prev_page:
                    ...
                    self.currenturi = prev_page['href']
                    res=self._getHTML()
                    self.rawpage=res['result']
                    self._setCurrentPage()
                else:
