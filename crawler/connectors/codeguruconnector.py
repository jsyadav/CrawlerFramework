...
log = logging.getLogger('CodeGuruConnector')

class CodeGuruConnector(BaseConnector):
    '''
    
    Sample uris is
    http://www.codeguru.com/forum/forumdisplay.php?f=81
    '''
    @logit(log , 'fetch')
    def fetch(self):
        """
        Fetch of http://www.codeguru.com/forum/
        """
        self.genre="Review"
        try:
            ...
            log.info(self.parent_uri)
            if self.currenturi.startswith('http://www.codeguru.com/forum/showthread.'):
                ...
                while True:
                    self.__addPosts()
                    try:
                        self.currenturi = self.__getStandUri('http://www.codeguru.com/forum/' + self.soup.find('a',text='&gt;').parent['href'])
                    except:
                    ...
                    if not self.__setSoup():
                        ...
                        break
                return True
            elif self.currenturi.startswith('http://www.codeguru.com/forum/forumdisplay'):
                self.total_posts_count = 0
                self.last_timestamp = datetime( 1980,1,1 )
                self.max_posts_count = int(tg.config.get(path='Connector',key='codeguruforum_numresults'))
    ...
    def __getThreads( self ):
        ...
        for thread in threads:
                ...
                try:
                    ...
                    temp_task.instance_data[ 'uri' ] = normalize( 'http://www.codeguru.com/forum/' + thread.find('a',id=re.compile('^thread_title_.*$'))['href'] )
    ...
    def __getParentPage(self):
        ...
        try:
            page['et_thread_id'] = self.parent_uri.split('&')[-1].split('=')[-1]
        except:
