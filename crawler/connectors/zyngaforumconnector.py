...
log = logging.getLogger('ZyngaForumConnector')

class ZyngaForumConnector(BaseConnector):
    '''
    Sample uris is
    http://forums.zynga.com/forumdisplay.php?f=17
    '''
    def fetch(self):
        """
        Fetch of zynga forums
        """
        self.genre="Review"
        try:
            self.parent_uri = self.currenturi
            if self.currenturi.startswith('http://forums.zynga.com/showthread.'):
                self.post_type= True
                if not self.__setSoup():
                    log.info(self.log_msg('Soup not set ,Returning False from Fetch'))
                    return False
                self.__getParentPage()
                sess_key = ('last_crawled_page',)
                current_page_no = 1
                if self.session_info_out and not self.task.instance_data.get('update'):
                    last_crawled_page_hash = self.session_info_out.get( sess_key )
                    if last_crawled_page_hash:
                        current_page_no = int(last_crawled_page_hash['hash'])
                        log.info(self.log_msg('The page was crawled till %d'%current_page_no))
                if current_page_no>1:
                    self.post_type = False
                    self.currenturi = self.parent_uri + '&page=%d'%current_page_no
                    if not self.__setSoup():
                        return False
                while True:
                    self.__addPosts()
                    result = updateSessionInfo('review', self.session_info_out ,'last_crawled_page',str(current_page_no),'Review',self.task.instance_data.get('update'))
                    if result['updated']:
                        log.info(self.log_msg('Last crawled date is updated'))
                    try:
                        self.currenturi = 'http://forums.zynga.com/' + self.soup.find('a',text='&gt;').parent['href']
                        current_page_no = dict(parse_qsl(self.currenturi.split('?')[-1]))['page']
                    except:
                    ...
                    if not self.__setSoup():
                        ...
                        break
                return True
            elif self.currenturi.startswith('http://forums.zynga.com/forumdisplay'):
                self.total_posts_count = 0
                self.last_timestamp = datetime( 1980,1,1 )
                self.max_posts_count = int(tg.config.get(path='Connector',key='zynga_forum_numresults'))
    ...
    def __getThreads(self):
        ''' It will get the list of thread urls and add the tasks
        '''
        try:
        ...
        for thread in threads:
            ...
            try:
                ...
                temp_task.instance_data[ 'uri' ] = normalize( 'http://forums.zynga.com/' + thread.find('a',id=re.compile('^thread_title_.*$'))['href'] )
                ...
                log.exception( self.log_msg('task not found') )
                continue
        return True
    ...
    def __getData(self, review, post_type ):
            '''This will return the page dictionary of catptured fields
            '''
            page = {}
            try:
            ...
            try:
                url = 'http://forums.zynga.com' + review.find('a',id=re.compile ('^postcount'))['href']
                data_dict = dict(parse_qsl(url.split('?')[-1]))
                if 's' in data_dict.keys():
                    data_dict.pop('s')
                page['uri'] = 'http://forums.zynga.com/showpost.php?'+ urlencode( data_dict )
                page['et_data_post_type'] = post_type
                date_str  = stripHtml(review.find('td','thead').renderContents())\
                                                        .split('\n')[-1].strip()
                page['posted_date'] = datetime.strftime(datetime.strptime(date_str,\
                                        '%m-%d-%Y,  %I:%M %p'),"%Y-%m-%dT%H:%M:%SZ")
                page['title'] = stripHtml((review.find('td',id = re.compile\
    ...
    def __getParentPage(self):
            ...
            for each in ['et_last_post_author_name','ei_thread_replies_count','ei_thread_views_count','edate_last_post_date','ei_thread_votes_count','ef_thread_rating']:
                try:
