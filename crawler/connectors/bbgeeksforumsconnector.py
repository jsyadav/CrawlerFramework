...
log = logging.getLogger('BBGeeksForumsConnector')
class BBGeeksForumsConnector(BaseConnector):
    '''Forum for BB Geeks
    '''
    @logit(log , 'fetch')
    def fetch(self):
        ...
        try:
            ...
            if self.currenturi.startswith('http://forums.bbgeeks.com/forums'):
                self.__createTasksForThreads()
            if self.currenturi.startswith('http://forums.bbgeeks.com/threads'):
    ...
    def __addThreadAndPosts(self):
        ...
        try:
            ...
            while self.__iteratePosts():
                try:
                    self.currenturi = self.__removeSessionId('http://forums.bbgeeks.com/' + self.soup.find('a', rel='prev').parent['href'])
                    self.__setSoupForCurrentUri()
                except:
    ...
    def __createTasksForThreads(self):
        ...
        self.__max_threads_count = int(tg.config.get(path='Connector', key=\
                                            'bbgeeksforums_maxthreads'))
        
        while self.__getThreads():
            try:
                self.currenturi = self.__removeSessionId('http://forums.bbgeeks.com/' + self.soup.find('img', alt='Next').parent['href'])
                self.__setSoupForCurrentUri()
            except:
    ...
    def __getThreads(self):
            ...
            for thread in threads:
                ...
                try:
                    self.__links_to_process.append( self.__removeSessionId('http://forums.bbgeeks.com/' + thread.find('h3', 'threadtitle').a['href'] ))
                except:
    ...
    def __addPost(self, post, is_question = False):
        ...
        try:            
            ...
            unique_key = 'http://forums.bbgeeks.com/' + unique_tag['href']
    ...
    def __getData(self, post, is_question, permalink):
        ...
        try:
            ...
            page['title'] = stripHtml(self.soup.find('span', 'threadtitle').renderContents())
            if not is_question:
