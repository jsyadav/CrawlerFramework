...
log = logging.getLogger('HTCForumsConnector')
class HTCForumsConnector(BaseConnector):
    '''Forum for BB Geeks
    '''
    @logit(log , 'fetch')
    def fetch(self):
    ...
    def __addThreadAndPosts(self):
        ...
        try:
            ...
            question_post = self.soup.find('ol',id='posts').find('li' , id=re.compile('post_\d+')).extract()
