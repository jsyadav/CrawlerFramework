...
log = logging.getLogger('MrExcelConnector')

class MrExcelConnector(BaseConnector):
    '''

    Sample uris is
    http://www.mrexcel.com/forum/forumdisplay.php?f=18
    '''
    @logit(log , 'fetch')
    def fetch(self):
        """
        Fetch of http://www.mrexcel.com/forum
        """
        self.genre="Review"
        try:
            ...
            if self.currenturi.startswith('http://www.mrexcel.com/forum/showthread.'):
                ...
                while True:
                    self.__addPosts()
                    try:
                        self.currenturi = self.__getStandUri('http://www.mrexcel.com/forum/' + self.soup.find('a',text='&gt;').parent['href'])
                    except:
                    ...
                    if not self.__setSoup():
                        ...
                        break
                return True
            elif self.currenturi.startswith('http://www.mrexcel.com/forum/forumdisplay'):
                self.unique_urls =[]
                self.total_posts_count = 0
                self.last_timestamp = datetime( 1980,1,1 )
                self.max_posts_count = int(tg.config.get(path='Connector',key='mrexcel_max_threads_to_process'))
    ...
    def __getThreads( self ):
        ...
        for thread in threads:
                ...
                try:
                    ...
                    if post_date.startswith('Today') or post_date.startswith('Yesterday'):
                        self.total_posts_count = self.total_posts_count - 1
                        log.info(self.log_msg('Today s post or yesterday post'))
                        continue
                    post_date = re.sub("(\d+)(st|nd|rd|th)",r"\1",post_date).strip()
                    thread_time = datetime.strptime(post_date,'%b %d, %Y  %I:%M %p')
                    ...
                    task_uri = self.__getStandUri( 'http://www.mrexcel.com/forum/' + thread.find('a',id=re.compile('^thread_title_.*$'))['href'] )
                    if task_uri in self.unique_urls:
                        self.total_posts_count = self.total_posts_count - 1
                        log.info(self.log_msg('Alread Available'))
                        continue
                    else:
                        self.unique_urls.append(task_uri)
                    temp_task=self.task.clone()
                    temp_task.instance_data[ 'uri' ] =   task_uri
    ...
    def __getData(self, review, post_type ):
        ...
        #page = {}
        try:
            page['title'] = stripHtml(review.find('td',id=re.compile('^td_post_.*?')).find('div',{'class':'smallfont'}).renderContents())
        except:
        ...
        try:
            member_ship = stripHtml(review.find('a','bigusername').findNext('div','smallfont').renderContents())
            if not member_ship.startswith('Join Date:') and not member_ship=='' :
                page['et_author_membership'] = member_ship
        except:
        ...
        try:
            ...
            page['et_thread_id'] = self.thread_id
        except:
    ...
    def __getParentPage(self):
        page = {}
        try:
            self.thread_id =  page['et_thread_id'] = self.parent_uri.split('&')[-1].split('=')[-1]
        except:
