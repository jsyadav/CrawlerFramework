...
log = logging.getLogger('HTCCommunityConnector')
class HTCCommunityConnector(BaseConnector):
    '''Forum for BB Geeks
    '''
    @logit(log , 'fetch')
    def fetch(self):
        """Fetch of http://community.htc.com
        """
        try:
            self.__links_to_process = []
            #self.__is_question = True
            if '/f/' in self.currenturi and not '/t' in self.currenturi :
                self.__createTasksForThreads()
            if '/t/' in self.currenturi and '/f' in self.currenturi:
    ...
    def __addThreadAndPosts(self):
        ...
        try:
            ...
            question_post = self.soup.find('div','full-post-container').extract()
            self.__addPost(question_post, True)  
            self.__goToLastPage()                 
            while self.__iteratePosts():
                try:
                    self.currenturi = 'http://community.htc.com' + self.soup.find('a','previous')['href']
                    self.__setSoupForCurrentUri()
                except:
    ...
    def __createTasksForThreads(self):
        ...
        self.__max_threads_count = int(tg.config.get(path='Connector', key=\
                                            'htccommunity_maxthreads'))
        
        while self.__getThreads():
            try:
                self.currenturi = 'http://community.htc.com' + self.soup.find('span' , text='Next').parent.parent['href']
                self.__setSoupForCurrentUri()
            except:
    ...
    def __getThreads(self):
            ...
            and add the tasks
            """
            threads = [x for x in self.soup.find('table', 'table-list').find('tbody').findAll('tr', recursive=False) if x.find('h4', 'post-name')]
            if not threads:
            ...
            for thread in threads:
                ...
                if self.__total_threads_count > self.__max_threads_count:
                    ...
                    return False
                try:
                    date_str = stripHtml(thread.find('div', 'post-date').renderContents())
                    thread_time = datetime.strptime(date_str, '%d %b %Y %I:%M %p')
                except:
                ...
                try:
                    self.__links_to_process.append('http://community.htc.com' + thread.find('h4', 'post-name').a['href'])
                except:
    ...
    def __iteratePosts(self):
        ...
        """
        try:
            posts = [x.findParent('li') for x in self.soup.findAll('div', 'post-date')]
            posts.reverse()
            log.info(self.log_msg('Total # of msg is %d'%len(posts)))
            if not posts:
    ...
    def __goToLastPage(self):
        ...
        """
        try:
            pagination_tag = self.soup.find('div','pager')
            if not pagination_tag:
                return
            uri = None
            last_page_tag  = pagination_tag.findAll('a','page')[-1]
            if last_page_tag:
                uri = last_page_tag['href']
            if not uri:
                log.info(self.log_msg('Post found in only one page'))
                return
            self.currenturi = 'http://community.htc.com' + uri
            self.__setSoupForCurrentUri()
        except:
    ...
    def __getData(self, post, is_question):
        ...
        try:
            page['data'] = stripHtml(post.find('div', 'post-content user-defined-markup').renderContents())
            page['title'] = stripHtml(self.soup.find('h4', 'post-name').renderContents())
            if not is_question:
        ...
        if not page['data']: 
            ...
            return False 
        try:
            page['et_author_name'] = stripHtml(post.find('span', 'user-name').renderContents())
        except:
            log.info(self.log_msg('author name not found'))
        try:
            date_str = stripHtml(post.find('div', 'post-date').find('span', 'value').renderContents())
