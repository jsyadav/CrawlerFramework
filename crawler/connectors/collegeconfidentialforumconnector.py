...
log = logging.getLogger('CollegeConfidentialConnector')

class CollegeConfidentialForumConnector(BaseConnector):
    '''
    Sample uris is http://talk.collegeconfidential.com/college-admissions/
    '''
    @logit(log , 'fetch')
    def fetch(self):
        """
        ...
        try:
            ...
            if rcheck.search(self.currenturi):
                ...
                self.max_posts_count = int(tg.config.get(path='Connector',key='collegeconfidential_numresults'))
    ...
    def __getThreads( self ):
        ...
        for thread in threads:
                ...
                try:
                    ...
                    thread_replies_soup = thread.find('td',title=re.compile('^Replies.*'))
                    if not thread_replies_soup:
                        log.debug(self.log_msg("Thread with title:%s has been Moved. Ignoring this thread" %thread_title))
                        continue
                    last_post_and_author = [each.strip() for each in stripHtml(thread_replies_soup.find('div').renderContents()).split('\n') if not each =='']
                    ...
                    temp_task.instance_data[ 'uri' ] = normalize(thread.find('a',id=re.compile('^thread_title_.*$'))['href'] )
                    ...
                    log.info(self.log_msg('the thread title is:'))
                    log.info(temp_task.pagedata['title'])
                    try:
    ...
    def __getData(self, review, post_type ):
        ...
        try:
            ...
            if date_str.startswith('Today'):
                date_str = date_str.replace('Today',datetime.strftime(datetime.utcnow(),'%m-%d-%Y'))
        ...
        try:
            ...
            page['et_data_forum'] = hierarchy[0].split('    ')[1]
            page['et_data_subforum'] = hierarchy[0].split('    ')[2]
            page['et_data_topic'] = hierarchy[1]
        except:
            log.info(self.log_msg('data forum not found'))
        try:
            if post_type=="Question":
                page['title'] = page['et_data_topic']
        except:
            log.info(self.log_msg('Question title not found'))
        return page
