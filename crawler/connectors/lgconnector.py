...
log = logging. getLogger("LgConnector")

class LgConnector(BaseConnector):    
    """Connector for lg.com 
    """
    @logit(log,'fetch')
    def fetch(self):
        """
        It fetches the data from the url mentioned
        sample url :http://www.lg.com/us/tv-audio-video/televisions/LG-led-tv-47LW6500.jsp
        """
        self.__genre = "Review"
        ...
        try:
            ...
            self.currenturi = self.soup.find('iframe',id = 'BVReviewsNoscript')['src']
            self.__setSoupForCurrentUri()
